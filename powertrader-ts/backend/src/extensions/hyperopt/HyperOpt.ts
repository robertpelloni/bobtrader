...
export class HyperOpt {
    public async optimize(config: HyperOptConfig): Promise<any> {
        ...
        for (let gen = 0; gen < config.generations; gen++) {
            ...
            // Mutate (10% rate)
            const mutatedPop: any[] = [];
            const currentPop = population as any[];
            for (let k = 0; k < currentPop.length; k++) {
                const ind = currentPop[k];
                mutatedPop.push(this.mutate(ind, config.parameterSpace, 0.1));
            }
