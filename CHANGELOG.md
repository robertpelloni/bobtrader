...
### Added
- **Coinbase Connector**: Full support for Coinbase Advanced Trade API with HMAC-SHA256 authentication.
- **Real-Time Core**: `Trader` now broadcasts `TRADE_UPDATE` and `ACCOUNT_UPDATE` events via WebSockets to the frontend.
- **Unit Testing**: Added Jest test suite for `Trader` logic (Entry, DCA, Exit) and mocked Exchange Connectors.
