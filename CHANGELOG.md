...
### Added
- **System Status Dashboard**: New frontend page (`/status`) showing module health, exchange status, and project structure.
- **Unified Documentation**: Consolidated agent instructions into `UNIVERSAL_LLM_INSTRUCTIONS.md`.
- **Roadmap Update**: Added milestones for v3.2.0 (DeFi Liquidity) and Mobile App.

## [3.1.0] - 2026-02-10

### Added
- **DeepThinker AI**: LSTM-based neural network engine using `@tensorflow/tfjs-node` for price prediction.
- **DeFi Integration**: `UniswapConnector` for decentralized trading on EVM chains (Polygon/Ethereum).
- **AI Lab UI**: New frontend page for interactive model training and inference visualization.
- **Configuration**: Added `defi` section and `active_ai` selector to `config.yaml` schema.

## [2.7.0] - 2026-01-18

### Added
- **Multi-Exchange Trading**: Implemented full `createOrder`, `cancelOrder`, and `fetchBalance` for KuCoin and Binance with HMAC-SHA256 signing.
- **Frontend Backtesting UI**: `StrategySandbox.tsx` now calls the real backend API and visualizes Equity Curve, Win Rate, and Drawdown.
- **Dynamic Configuration**: `ConfigManager` updated to support `exchanges.kucoin` and `exchanges.binance` credentials.

## [2.6.0] - 2026-01-18

### Added
- **Notification System**: Full TypeScript port of Email, Discord, and Telegram notifications (integrated into Trader).
- **Paper Trading Engine**: Configurable `execution_mode` (live/paper) with realistic fee/slippage simulation.
- **Historical Backtesting**: Robust `BacktestEngine` for deep historical simulations with Sharpe Ratio and Drawdown analysis.
- **Genetic Optimization (HyperOpt)**: Implemented Genetic Algorithm to auto-tune strategy parameters (`RSIStrategy`, `SMAStrategy`).
- **Data Engine**: Enhanced `HistoricalData` module to fetch and paginate long histories from KuCoin.
