...
func TestLoadFileOverridesDefault(t *testing.T) {
	...
	content := `{"environment":"test","event_log":{"path":"tmp/events.jsonl"},"snapshots":{"path":"tmp/snapshots.jsonl"},"orders":{"path":"tmp/orders.jsonl"},"logging":{"path":"tmp/app.jsonl","stdout":false},"server":{"enabled":false,"address":"127.0.0.1:9191"},"scheduler":{"enabled":true,"interval_ms":500},"risk":{"max_notional":250,"allowed_symbols":["BTCUSDT"],"cooldown_ms":1000,"duplicate_window_ms":2000},"accounts":[{"id":"acct-1","name":"Test","enabled":true,"exchange":"paper","capabilities":["spot"]}]}`
	...
	if cfg.Risk.MaxNotional != 250 || cfg.Risk.CooldownMS != 1000 || cfg.Risk.DuplicateWindowMS != 2000 || len(cfg.Risk.AllowedSymbols) != 1 || cfg.Risk.AllowedSymbols[0] != "BTCUSDT" {
