...
type stubTickRunner struct{ calls atomic.Int32 }

func (r *stubTickRunner) RunTick(ctx context.Context, tick marketdata.Tick) error {
	r.calls.Add(1)
	return nil
}

func TestReportingTickRunnerWritesReports(t *testing.T) {
	store, err := reports.NewStore(filepath.Join(t.TempDir(), "reports.jsonl"))
	if err != nil {
		t.Fatalf("NewStore returned error: %v", err)
	}
	runner := &stubTickRunner{}
	reportingRunner := NewReportingTickRunner(runner, store, func(ctx context.Context) []reports.Report {
	...
	if runner.calls.Load() != 1 {
		t.Fatalf("expected underlying tick runner to be called once, got %d", runner.calls.Load())
	}
