...
class RobinhoodMarketData:
    def __init__(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to decode Robinhood private key (r_secret.txt): {e}"
            )
    ...
    def get_current_ask(self, symbol: str) -> float:
        ...
        if not data or "results" not in data or not data["results"]:
            raise RuntimeError(
                f"Robinhood best_bid_ask returned no results for {symbol}: {data}"
            )
...
def step_coin(sym: str):
    ...
    if tf_choice_index >= len(tf_choices):
        ...
        from pt_thinker_exchanges import (
        ...
        while True:
            try:
                ...
                arb_opp = detect_arbitrage_opportunities(sym, min_spread_pct=0.3)

                if arb_opp:
                    print(f"\n[ARBITRAGE OPPORTUNITY]")
                    print(f"  Coin: {arb_opp['coin']}")
                    print(
                        f"  Buy: {arb_opp['buy_exchange']} @ ${arb_opp['buy_price']:,.2f}"
                    )
                    print(
                        f"  Sell: {arb_opp['sell_exchange']} @ ${arb_opp['sell_price']:,.2f}"
                    )
                    print(f"  Spread: {arb_opp['spread_pct']:.2f}%")
...
try:
    while True:
        ...
        from pt_thinker_exchanges import init_exchanges
